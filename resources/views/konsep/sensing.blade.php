        ...
        <div class="container mx-auto px-4 pt-28 pb-16">
            <div class="max-w-4xl mx-auto">
                <h1 class="text-4xl font-bold text-gray-900 text-center mb-8">
                    {{ $concept->title ?? 'Mesin Kecerdasan Sensing' }}</h1>
                <div class="rounded-2xl overflow-hidden shadow-md mb-10">
                    @if (isset($concept) && $concept->image_path)
                        @if (str_starts_with($concept->image_path, 'uploads/'))
                            <img src="{{ Storage::url($concept->image_path) }}" alt="{{ $concept->title ?? 'Sensing' }}"
                                class="w-full h-72 md:h-[420px] object-cover">
                        @else
                            <img src="{{ asset('img/' . $concept->image_path) }}" alt="{{ $concept->title ?? 'Sensing' }}"
                                class="w-full h-72 md:h-[420px] object-cover">
                        @endif
                    @else
                ...
                <div class="prose max-w-none">
                    ...
                    @else
                        <p class="text-gray-700 leading-8">Tipe Sensing memproses informasi melalui pengalaman langsung dan
                            detail konkret. Mereka cenderung pragmatis, realistis, dan sangat memperhatikan hal-hal teknis
                            yang
                            bisa diterapkan.</p>
                        ...
                        <p class="text-gray-700 leading-8">Melatih fleksibilitas, berpikir abstrak, serta memperkuat
                            kemampuan
