        ...
        <div class="container mx-auto px-4 pt-28 pb-16">
            <div class="max-w-4xl mx-auto">
                <h1 class="text-4xl font-bold text-gray-900 text-center mb-8">
                    {{ $concept->title ?? 'Mesin Kecerdasan Intuiting' }}</h1>
                <div class="rounded-2xl overflow-hidden shadow-md mb-10">
                    @if (isset($concept) && $concept->image_path)
                        @if (str_starts_with($concept->image_path, 'uploads/'))
                            <img src="{{ Storage::url($concept->image_path) }}" alt="{{ $concept->title ?? 'Intuiting' }}"
                                class="w-full h-72 md:h-[420px] object-cover">
                        @else
                            <img src="{{ asset('img/' . $concept->image_path) }}" alt="{{ $concept->title ?? 'Intuiting' }}"
                                class="w-full h-72 md:h-[420px] object-cover">
                        @endif
                    @else
                        <img src="{{ asset('img/hero.png') }}" alt="Intuiting"
                            class="w-full h-72 md:h-[420px] object-cover">
                    @endif
                </div>
                <div class="prose max-w-none">
                    @if (isset($concept) && $concept->content)
                        {!! $concept->content !!}
                    @else
                        <p class="text-gray-700 leading-8">Tipe Intuiting berfokus pada pola, kemungkinan masa depan, dan
                            visi
